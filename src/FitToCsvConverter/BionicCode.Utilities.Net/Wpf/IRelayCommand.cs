...
/// <inheritdoc/>
public interface IRelayCommand : IRelayCommandCommon
{
    /// <summary>
    ...
    /// <br/>This is the default behavior. <see cref="CommandManager.InvalidateRequerySuggested"/> is regularly invoked by the framework to keep UI elements, that implement <see cref="ICommandSource"/> (like the Button control) and listen to the <see cref="ICommand.CanExecuteChanged"/> event, updated. 
    /// <br/>When the <see cref="IsCommandManagerRequerySuggestedEnabled"/> property is set to <c>false</c>, the <see cref="ICommand.CanExecuteChanged"/> must be raised explicitly by calling the <see cref="IAsyncRelayCommandCore.InvalidateCommand"/> method.</remarks>
